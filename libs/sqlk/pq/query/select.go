...
func newSelectBuilder() *SelectBuilder {
	return &SelectBuilder{
		fields:    []sqlk.SelectWriter{},
		orderBys:  []sqlk.OrderByWriter{},
...
func (b *SelectBuilder) Select(column1 sqlk.SelectWriter, columnN ...sqlk.SelectWriter) *SelectBuilder {
	// Set existing
	b.fields = append([]sqlk.SelectWriter{column1}, columnN...)
	return b
}
...
func (b *SelectBuilder) Where(w1 sqlk.WhereWriter, wn ...sqlk.WhereWriter) *SelectBuilder {
	...
	where := append([]sqlk.WhereWriter{w1}, wn...)
...
func (b *SelectBuilder) writeSelectQuery() string {
	...
	return strings.Join(fields, sqlk.Separator)
}
...
func (b *SelectBuilder) writeOrderByQuery() string {
	...
	var writers []sqlk.OrderByWriter
	for _, f := range b.orderBys {
		// Get existing table, if not then filter out writer
		sRef := f.GetSchemaRef()
		table, ok := b.schemaRef[sRef]
	...
	q := strings.Join(arr, sqlk.Separator)
